...
public interface CarRepo extends JpaRepository<Car,String> {
    ...
    List<Car> getAvailableCarsForReservation(String pick_up_date, String return_date);

    @Query(value = "SELECT * FROM Car WHERE availability='Available' AND registrationId NOT IN (SELECT DISTINCT registrationId FROM Rental WHERE (reservation_status='Pending' OR reservation_status='Accept') AND ?1 BETWEEN pick_up_date AND return_date) ", nativeQuery = true)

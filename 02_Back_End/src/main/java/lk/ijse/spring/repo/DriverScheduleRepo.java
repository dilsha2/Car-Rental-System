...
public interface DriverScheduleRepo extends JpaRepository<DriverSchedule, String> {
    @Query(value = "SELECT * FROM driver_schedule WHERE reserve_id NOT IN(SELECT reserve_id FROM Rental WHERE reservation_status='Done' OR reservation_status='Deny') AND (start_date BETWEEN ?1 AND ?2 OR end_date BETWEEN ?1 AND ?2)", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM driver_schedule WHERE reserve_id NOT IN(SELECT reserve_id FROM Rental WHERE reservation_status='Done' OR reservation_status='Deny') AND (driver_nic=?1) AND (start_date BETWEEN ?2 AND ?3 OR end_date BETWEEN ?2 AND ?3)", nativeQuery = true)

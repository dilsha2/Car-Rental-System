...
$("#admin-payment-searchBtn").click(function () {
    ...
    $.ajax({
        url: baseUrl + "reservation/getReservation/" + reserve_id,
        method: "GET",
        success: function (resp) {
            if (resp.code === 200) {
                data = resp.data;
...
function calculateTotal() {
    ...
    if (days >= 30) {
        rate = data.car.monthlyRate
        extra_km = totalKm - data.freeKmMonth
    } else {
        rate = data.car.dailyRate * days
        extra_km = totalKm - data.car.freeKmDay * days
    }

    if (extra_km > 0) {
        totalPrice = rate + (extra_km * data.car.priceForExtraKm);
    } else {
        totalPrice = rate
    }
